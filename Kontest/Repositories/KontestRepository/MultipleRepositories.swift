...
final class MultipleRepositories: KontestFetcher {
    /// Logger instance for logging events related to the `MultipleRepositories` class.
    /// Logs are categorized under "MultipleRepositories" within the "com.ayushsinghal.Kontest" subsystem.
    ...
    /// Initializes the `MultipleRepositories` class with a list of repositories.
    ///
    ...
    func getAllKontests() async throws -> [KontestDTO] {
        ...
        if let lastError = lastError {
            ...
            throw NSError(domain: "MultipleRepositories", code: -1, userInfo: [NSLocalizedDescriptionKey: "No repositories available to fetch contests."])
        }
    }
}
